import streamlit as st

st.set_page_config(
    page_title="엑셀 서식 기반 계산기",
    page_icon="🧮",
    layout="centered",
)

st.markdown(
    """
    <style>
    .main-title {
        font-size: 34px;
        font-weight: 800;
        margin-bottom: 4px;
    }
    .sub-text {
        color: #64748b;
        margin-bottom: 24px;
    }
    .excel-table {
        border-collapse: collapse;
        width: 100%;
        background: white;
        box-shadow: 0 10px 30px rgba(15, 23, 42, 0.08);
        border-radius: 16px;
        overflow: hidden;
    }
    .excel-table th {
        background: #f8fafc;
        border: 1px solid #cbd5e1;
        padding: 12px;
        text-align: center;
        font-weight: 700;
    }
    .excel-table td {
        border: 1px solid #e2e8f0;
        padding: 12px;
        text-align: center;
        font-size: 18px;
    }
    .yellow-cell {
        background: #fef08a;
        font-weight: 700;
    }
    .label-cell {
        background: #f8fafc;
        font-size: 15px !important;
        font-weight: 700;
    }
    .result-box {
        background: #fef08a;
        padding: 12px;
        border-radius: 12px;
        font-size: 22px;
        font-weight: 800;
        text-align: right;
    }
    </style>
    """,
    unsafe_allow_html=True,
)


def calculate(a: float, op: str, b: float):
    if op == "+":
        return a + b
    if op == "-":
        return a - b
    if op == "×":
        return a * b
    if op == "/":
        if b == 0:
            return "0으로 나눌 수 없음"
        return a / b
    return ""


def format_result(value):
    if isinstance(value, str):
        return value
    if float(value).is_integer():
        return str(int(value))
    return f"{value:,.6f}".rstrip("0").rstrip(".")


st.markdown('<div class="main-title">엑셀 서식 기반 사칙연산 계산기</div>', unsafe_allow_html=True)
st.markdown(
    '<div class="sub-text">업로드된 엑셀 파일의 A / B / C 구조와 노란 입력 셀 스타일을 Python 웹앱으로 변환했습니다.</div>',
    unsafe_allow_html=True,
)

if "values" not in st.session_state:
    st.session_state.values = {
        "add_a": 1.0,
        "add_b": 1.0,
        "sub_a": 1.0,
        "sub_b": 1.0,
        "mul_a": 1.0,
        "mul_b": 1.0,
        "div_a": 1.0,
        "div_b": 1.0,
    }

if st.button("초기화", use_container_width=True):
    st.session_state.values = {
        "add_a": 1.0,
        "add_b": 1.0,
        "sub_a": 1.0,
        "sub_b": 1.0,
        "mul_a": 1.0,
        "mul_b": 1.0,
        "div_a": 1.0,
        "div_b": 1.0,
    }
    st.rerun()

rows = [
    ("add", "덧셈", "+"),
    ("sub", "뺄셈", "-"),
    ("mul", "곱셈", "×"),
    ("div", "나눗셈", "/"),
]

st.markdown("### 계산 입력")

header_cols = st.columns([1.1, 1.3, 0.7, 1.3, 0.5, 1.4])
for col, text in zip(header_cols, ["구분", "A", "연산", "B", "=", "C"]):
    col.markdown(f"**{text}**")

for row_id, label, op in rows:
    cols = st.columns([1.1, 1.3, 0.7, 1.3, 0.5, 1.4])
    cols[0].markdown(f"**{label}**")
    a = cols[1].number_input(
        f"{label} A",
        value=st.session_state.values[f"{row_id}_a"],
        key=f"{row_id}_a_input",
        label_visibility="collapsed",
    )
    cols[2].markdown(f"<div style='text-align:center;font-size:24px;font-weight:800'>{op}</div>", unsafe_allow_html=True)
    b = cols[3].number_input(
        f"{label} B",
        value=st.session_state.values[f"{row_id}_b"],
        key=f"{row_id}_b_input",
        label_visibility="collapsed",
    )
    cols[4].markdown("<div style='text-align:center;font-size:24px;font-weight:800'>=</div>", unsafe_allow_html=True)
    result = calculate(a, op, b)
    cols[5].markdown(f"<div class='result-box'>{format_result(result)}</div>", unsafe_allow_html=True)
    st.session_state.values[f"{row_id}_a"] = a
    st.session_state.values[f"{row_id}_b"] = b

st.divider()

st.markdown("### 엑셀 수식 대응")
st.info(
    "엑셀의 `=B2+D2`, `=B4-D4`, `=B6*D6`, `=B8/D8` 구조를 Python 계산 로직으로 변환했습니다."
)

st.markdown("### 실행 방법")
st.code("streamlit run app.py", language="bash")
